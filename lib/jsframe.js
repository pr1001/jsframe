var jsFrameObject = Class.create({
	initialize: function()
	{
		...
		// add FireBug-style logging
		// add console object if it doesn't already exist
		window.console = window.console || {};
		// if Opera and not console.log
		if (typeof(console.log) == 'undefined' && typeof(opera) != 'undefined' && typeof(opera.postError))
		{
	...
	render: function()
	{
		...
		// get template
		console.log('Now we would get the template');
		// write script		
		var s = new Element('script');
		s.type = 'text/javascript';
		s.src = 'views/Templates/' + this.rendering.template + '.js';
		$$('head').insert({bottom: s});
...
/*
		new Ajax.Request('views/Templates/' + this.rendering.template + '.js', {
			...
			onSuccess: function(transport) {
				...
				// get view
				console.log('Now we would get the view');
